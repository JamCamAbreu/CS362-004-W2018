...
// Village is suppose to:
// 1. +1 card
// 2. +2 actions
// 3. discard village card
...
int main(int argc, char* argv[]) {
  ...
  if (DISPLAY_TESTS == 1)
    printf("VILLAGE RANDOM TEST: Begin test with seed=%d\n", seed);
  ...
  int villageLoc = 0; // the hand location of the adventurer card
  int stHandNum;
  int stActionsNum;
  ...
  // Replace first card in players hand with a village card
  ...
  playVillage(&G, p, villageLoc);

  G.numActions--; // for the VILLAGE card
